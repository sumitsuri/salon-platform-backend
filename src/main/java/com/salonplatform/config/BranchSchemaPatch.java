...
public class BranchSchemaPatch implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            jdbcTemplate.update(
                    "UPDATE branches SET business_type = 'SALON_AND_SPA' WHERE code = 'VAR' AND (business_type IS NULL OR business_type = '')");
            jdbcTemplate.update(
                    "UPDATE branches SET business_type = 'SALON' WHERE business_type IS NULL OR business_type = ''");
            jdbcTemplate.update(
            ...
            jdbcTemplate.update(
                    "UPDATE branches SET monthly_sales_target = 320000 WHERE code = 'ALP' AND monthly_sales_target IS NULL");
            jdbcTemplate.update(
